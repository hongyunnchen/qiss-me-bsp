...
#{add by Henry,for bootimg ota incr update,2012-04-18
    def UpdateBoot(self):
      self.script.append(
            ('package_extract_file("update_bootimg.sh", "/tmp/update_bootimg.sh");\n'
             'set_perm(0, 0, 0777, "/tmp/update_bootimg.sh");'))
      self.script.append(
            ('package_extract_file("busybox", "/tmp/busybox");\n'
             'set_perm(0, 0, 0777, "/tmp/busybox");'))
      self.script.append(
      ...
      self.script.append(
            ('package_extract_file("bml_over_mtd.sh", "/tmp/bml_over_mtd.sh");\n'
             'set_perm(0, 0, 0777, "/tmp/bml_over_mtd.sh");'))
      self.script.append('package_extract_file("boot.img", "/tmp/boot.img");')
      self.script.append('assert(run_program("/tmp/update_bootimg.sh") == 0);')
# add by Henry,for bootimg ota incr update,2012-04-18}
