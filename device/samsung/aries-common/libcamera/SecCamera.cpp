...
static int fimc_v4l2_s_ext_ctrl(int fp, unsigned int id, void *value)
{
    ...
    ctrl.id = id;
    ctrl.string = (char *) value;//Brian TODO new kernel struct to be modify
...
int SecCamera::stopRecord(void)
{
    ...
    if (m_camera_id == CAMERA_ID_BACK) {
        ...
        CHECK(ret);
        /* { Added by Jerry.Qian, Camera can not capture successfully after switch from camcorder to Photo, 2012/10/16 */
...
int SecCamera::setSnapshotCmd(void)
{
    ...
    CHECK(ret);
    /* { Added by Jerry.Qian, for ucam, 2012/07/13 */
    mLock.lock();
    /* Added by Jerry.Qian, for ucam, 2012/07/13 } */
    /* { Added by Jerry.Qian, do the first two steps of kernel capture, 2012/06/28 */
