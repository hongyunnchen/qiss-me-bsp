...
void CameraHardwareSec::initDefaultParameters(int cameraId)
{
    ...
    if (cameraId == SecCamera::CAMERA_ID_BACK) {
    	//Brian for MMS capture resolution start
        p.set(CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES,
              "1280x720,800x480,720x480,640x480,592x480,352x288,176x144");
    	//Brian for MMS capture resolution end
...
//    sp<MemoryBase> buffer = new MemoryBase(mRawHeap, 0, mPostViewSize + 8);
    ...
    if (mSecCamera->getCameraId() == SecCamera::CAMERA_ID_BACK) {
        /* { Added by Jerry.Qian, re-alloc the memory if jpeg size is larger, 2012/06/18 */
        if(jpeg_size > mJpegHeapSize) {
            JpegHeap->release(JpegHeap);
            JpegHeap = mGetMemoryCb(-1, jpeg_size, 1, 0);
        }
        /* Added by Jerry.Qian, re-alloc the memory if jpeg size is larger, 2012/06/18 } */
...
static struct preview_stream_ops *buf_bk=NULL;
                                            ...
                                            const char *parms)
{
    ...
    int ret = 0; 
//Brian add for runtime preview resolution change start
    ...
    if(preview_resolution_change && obj(dev)->previewEnabled())
    	{
    	obj(dev)->stopPreview();
    	}
    	
	ret = obj(dev)->setParameters(p);

    if(preview_resolution_change && obj(dev)->previewEnabled())
    	{
	obj(dev)->setPreviewWindow(buf_bk);
    	obj(dev)->startPreview();
    	}
//Brian add for runtime preview resolution change end
	return ret;
}
...
char *HAL_camera_device_get_parameters(struct camera_device *dev)
{
    ...
    if(parms.get(CameraParameters::KEY_PREVIEW_SIZE))
    	strcpy(previous_preview_size,parms.get(CameraParameters::KEY_PREVIEW_SIZE));
