...
#define UNPLUGGED_SHUTDOWN_TIME_1 (1 * MSEC_PER_SEC)
  ...
  //      .level_only = true,
   ...
   //     .min_capacity = BATTERY_FULL_THRESH,
    ...
    },
    {
        ...
        {
        .name = "charger/battery_8",
        .disp_time = 750,
    },
    {
        .name = "charger/battery_9",
        .disp_time = 750,
    },
    {
    ...
    {
        .name = "charger/battery_numbers/0",
	 .disp_time = 0,		
    },
    {
        .name = "charger/battery_numbers/1",
	 .disp_time = 0,			
    },
    {
        .name = "charger/battery_numbers/2",
	 .disp_time = 0,			
    },
    {
        .name = "charger/battery_numbers/3",
	 .disp_time = 0,			
    },
    {
        .name = "charger/battery_numbers/4",
	 .disp_time = 0,			
    },
    {
        .name = "charger/battery_numbers/5",
	 .disp_time = 0,			
    },
    {
        .name = "charger/battery_numbers/6",
	 .disp_time = 0,			
    },
    {
        .name = "charger/battery_numbers/7",
        .disp_time = 0,	
    },	
    {
        .name = "charger/battery_numbers/8",
	 .disp_time = 0,			
    },
    {
        .name = "charger/battery_numbers/9",
	 .disp_time = 0,			
    },
    {
        .name = "charger/battery_numbers/percentage",
	 .disp_time = 0,			
    }      
};
...
int show_percentage(struct charger *charger, int percentage)
{
	int w0;
	int h0;
	int x0;
	int y0;
	int base  = 12, i, offset;
	int times = 0;
	struct frame *frame ;
	gr_surface sur ;
	...
	do
	{
		...
		percentage = (percentage -i)/10;   		
    		frame = &charger->batt_anim->frames[base + i];
    		sur =  frame->surface ;
    		w0 = gr_get_width(sur);
		offset = times	* w0;
		times -= 1 ;
    		h0 = gr_get_height(sur);
    		x0 = (gr_fb_width() - w0) / 2  ;
    		y0 = gr_fb_height()-((gr_fb_height() - h0) / 4) ;
    		gr_blit(sur, 0, 0, w0, h0, x0+offset, y0);	
	}while(percentage != 0);

	frame = &charger->batt_anim->frames[base + 10];
	sur =  frame->surface ;
	offset = 1*w0;
	w0 = gr_get_width(sur);
	h0 = gr_get_height(sur);
...
int led_show(int capa_batt)
{
	unsigned int color = LED_BLANK;
	
			if(capa_batt < 0 || capa_batt > 100)
				color = LED_BLANK;
			else if(capa_batt <= 99)
				color =	LED_AMBER;
			else 	 
				color = LED_GREEN;
			switch (color) {
...
static int draw_surface_centered(struct charger *charger, gr_surface surface)
{
    ...
    batt_capa = get_battery_capacity(charger);
    //sprintf(batt_capa_s, "%d%%", batt_capa);
    //android_green();
    //draw_text(batt_capa_s, -1, 300); 	
    show_percentage(charger, batt_capa);	
    return y + h;
}
...
static void redraw_screen(struct charger *charger)
{
    ...
    {
    		fprintf(stderr,"kelcy capacity %d  num_frames %d\n",batt_anim->capacity, batt_anim->num_frames );	
...
static void update_screen_state(struct charger *charger, int64_t now)
{
    ...
    char on[] = "on" ;	
		...
		fprintf(stderr, "kelcy update_screen_state\n");
    ...
    if (batt_anim->cur_cycle == batt_anim->num_cycles) {
        ...
        fd = open("/sys/power/state", O_RDWR);
	 			if(fd < 0)
	 				fprintf(stderr, "open power state error\n");
	 
	 			write(fd,  &mem[0] , 3);
	 			comstat = 1 ;
	 			close(fd);
	 			fd = -1 ;
        fprintf(stderr, "update_screen_state 3 over\n");
    ...
    while (batt_anim->cur_frame < batt_anim->num_frames &&
        ...
        batt_anim->cur_frame++;
    if (batt_anim->cur_frame >= batt_anim->num_frames ||batt_anim->cur_frame >= 12 ) {
        batt_anim->cur_cycle++;
...
static void handle_power_supply_state(struct charger *charger, int64_t now)
{
    if (charger->num_supplies_online == 0) {
        if (charger->next_pwr_check == -1) {
	     ...
	     {
		 charger->next_pwr_check = now + UNPLUGGED_SHUTDOWN_TIME_1;
	     }	 
