...
#{Add by Hill.Hui,2012-05-31, for HD2 update
  def bootflashcmd(self):
#Brian for CLK/MAGLDR script start
	#self.script.append('mount("yaffs2", "MTD", "boot", "/boot");')
	#self.script.append('package_extract_dir("boot", "/boot");')
	#self.script.append('unmount("/boot");')
	self.script.append('package_extract_file("boot/checksys.sh","/tmp/checksys.sh");')
	self.script.append('set_perm(0, 0, 755, "/tmp/checksys.sh");')
	self.script.append('run_program("/tmp/checksys.sh");')
	self.script.append('if file_getprop("/tmp/nfo.prop","clk") == "true" then')
	self.script.append('  ui_print("cLK Bootloader found.");' )
	self.script.append('  ui_print("Flashing cLK kernel...");')
	self.script.append('  assert(')
	self.script.append('    package_extract_file("boot/initrd.gz", "/tmp/initrd.gz"),')
	self.script.append('    package_extract_file("boot/zImage", "/tmp/zImage"),')
	self.script.append('    package_extract_file("boot/mkbootimg","/tmp/mkbootimg"),')
	self.script.append('    package_extract_file("boot/mkbootimg.sh","/tmp/mkbootimg.sh"));')
	self.script.append('  set_perm(0, 0, 0755, "/tmp/mkbootimg");')
	self.script.append('  set_perm(0, 0, 0755, "/tmp/mkbootimg.sh");')
	self.script.append('  assert(')
	self.script.append('    run_program("/tmp/mkbootimg.sh"),')
	self.script.append('    write_raw_image("/tmp/boot.img", "boot"),')
	self.script.append('    delete("/tmp/boot.img"));')
	self.script.append('  #package_extract_dir("boot/clk-ppp-patch", "/system");')
	self.script.append('  #set_perm(0, 0, 0777, "/system/etc/init.d/97ppp");')
	self.script.append('else')
	self.script.append('  ui_print("MAGLDR Bootloader found.");')
	self.script.append('  ui_print("Flashing MAGLDR kernel...");')
	self.script.append('  mount("yaffs2", "MTD", "boot", "/boot");')
	self.script.append('  package_extract_dir("boot", "/boot");')
	self.script.append('  unmount("/boot");')
	self.script.append('  #delete("/system/ppp");')
	self.script.append('  #delete("/system/etc/init.d/97ppp");')
	self.script.append('endif;')
	self.script.append('#set_perm_recursive(0, 2000, 0755, 0750, "/system/etc/init.d");')
	self.script.append('#set_perm(0, 0, 0755, "/system/etc/init.d");')
	self.script.append('#set_perm_recursive(0, 1001, 0755, 0775, "/system/etc/ppp");')
	self.script.append('#set_perm(0, 0, 04755, "/system/bin/pppd");')
#Brian for CLK/MAGLDR script end