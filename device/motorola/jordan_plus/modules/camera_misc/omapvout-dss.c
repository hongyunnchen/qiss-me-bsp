...
/*
SYMSEARCH_DECLARE_FUNCTION_STATIC(void, ss_ispmmu_save_context, void);
SYMSEARCH_DECLARE_FUNCTION_STATIC(void, ss_ispmmu_restore_context, void);
SYMSEARCH_DECLARE_FUNCTION_STATIC(void, ss_isppreview_enable, int);
*/

#define DMA_CHAN_ALLOTED	1
#define DMA_CHAN_NOT_ALLOTED	0

#define VRFB_TX_TIMEOUT		10000

#define DEBUG 2
#undef DBG
#define DBG(fmt, args...) printk(KERN_INFO "dss %s:"fmt, __func__, ##args)

/*=== Local Functions ==================================================*/

static int cam_dss_format_bytespp(u32 pixelformat)
{
	int bpp = 2;
...
static enum omap_color_mode cam_dss_color_mode(u32 pixelformat)
{
...
static void cam_dss_calc_offset(struct omapvout_device *vout,
...
static int cam_dss_get_overlays(struct omap_overlay **gfx,
 ...
 *   already enabled.
 */
static void cam_dss_set_global_alpha(struct omapvout_device *vout)
{
	...
	if (cam_dss_get_overlays(&gfx, &vid1, &vid2)) {
 ...
 * enabled.
 */
static bool cam_dss_clr_global_alpha(struct omapvout_device *vout)
{
	struct omap_overlay *gfx, *vid1, *vid2;
	struct omap_overlay_info info;
	bool alpha_en = false;
...
static int cam_dss_enable_transparency(struct omapvout_device *vout)
{
...
static int cam_dss_disable_transparency(struct omapvout_device *vout)
{
	struct omap_overlay_manager *mgr;
	struct omap_overlay_manager_info m_info;
	...
	m_info.alpha_enabled = cam_dss_clr_global_alpha(vout);
	m_info.trans_enabled = false;
 ...
 */
static void cam_dss_vrfb_dma_cb(int lch, u16 ch_status, void *data)
{
...
int cam_dss_acquire_vrfb(struct omapvout_device *vout)
{
	int rc = 0;
	int size;
	int w, h;
	...
	rc = cam_mem_alloc(size, &vrfb->phy_addr[0], &vrfb->virt_addr[0]);
	if (rc != 0) {
		printk(KERN_ERR "VRFB buffer alloc 0 failed %d\n", rc);
		goto failed_mem0;
	}

	DBG("mem_alloc[0] pa/%08lx; va/%08x size=%d\n", vrfb->phy_addr[0], 
		(unsigned int) vrfb->virt_addr[0], vrfb->size);

	rc = cam_mem_alloc(size, &vrfb->phy_addr[1], &vrfb->virt_addr[1]);
	if (rc != 0) {
		printk(KERN_ERR "VRFB buffer alloc 1 failed %d\n", rc);
		goto failed_mem1;
	}

	DBG("mem_alloc[1] pa/%08lx; va/%08x\n", vrfb->phy_addr[1], (unsigned int) vrfb->virt_addr[1]);
...
failed_dma:
	cam_mem_free(vrfb->phy_addr[1], vrfb->virt_addr[1], size);
failed_mem1:
	cam_mem_free(vrfb->phy_addr[0], vrfb->virt_addr[0], size);
failed_mem0:
...
static int cam_dss_release_vrfb(struct omapvout_device *vout)
{
	int rc = 0;
	int size;
	struct omapvout_dss_vrfb *vrfb;
	...
	if (vrfb->req_status == DMA_CHAN_ALLOTED) {
		...
		DBG("mem_free[0] pa/%08lx; va/%08x; sz=%d\n", vrfb->phy_addr[0], (unsigned int) vrfb->virt_addr[0], size);

		cam_mem_free(vrfb->phy_addr[0], vrfb->virt_addr[0], size);
		cam_mem_free(vrfb->phy_addr[1], vrfb->virt_addr[1], size);

		DBG("mem_free[1] pa/%08lx; va/%08x; sz=%d\n", vrfb->phy_addr[1], (unsigned int) vrfb->virt_addr[1], size);
...
static int cam_dss_perform_vrfb_dma(struct omapvout_device *vout,
...
/*
	if (0 == buf_idx % 2) {
		if (vrfb_cfg) {
			...
			omap_clear_dma(vrfb->dma_ch);
			DBG("ispmmu_save_context()");
			SYMSEARCH_BIND_FUNCTION_TO(camera, ispmmu_save_context, ss_ispmmu_save_context);
			ss_ispmmu_save_context();
			DBG("isppreview_enable()");
			SYMSEARCH_BIND_FUNCTION_TO(camera, isppreview_enable, ss_isppreview_enable);
			ss_isppreview_enable(0);
			ss_isppreview_enable(1);
		} else {
			DBG("ispmmu_restore_context()");
			SYMSEARCH_BIND_FUNCTION_TO(camera, ispmmu_restore_context, ss_ispmmu_restore_context);
			ss_ispmmu_restore_context();
		}
	}
*/
	if (vrfb_cfg) {
		...
		bytespp = cam_dss_format_bytespp(vout->pix.pixelformat);
		dss_fmt = cam_dss_color_mode(vout->pix.pixelformat);
		omap_vrfb_setup(&vrfb->ctx[0], vrfb->phy_addr[0],
				w, h, bytespp, is_yuv, rot);
		omap_vrfb_setup(&vrfb->ctx[1], vrfb->phy_addr[1],
				w, h, bytespp, is_yuv, rot);
	
		DBG("vrfb_cfg pa[0]/%08lx pa[1]/%08lx\n", vrfb->phy_addr[0], vrfb->phy_addr[1]);

		cam_dss_calc_offset(vout, vrfb->ctx[0].bytespp,
				vrfb->ctx[0].xoffset, vrfb->ctx[0].yoffset);

		DBG("vrfb_cfg dss offset=0x%x\n", vout->dss->foffset);
		...
		if (is_yuv) {
			vrfb->dst_fi = (OMAP_VRFB_LINE_LEN * bytespp * 2)
							- (vrfb->en * 4) + 1;
			DBG("vrfb_cfg yuv dst_fi=%d.", vrfb->dst_fi);
		} else {
			vrfb->dst_fi = (OMAP_VRFB_LINE_LEN * bytespp)
							- (vrfb->en * 4) + 1;
			DBG("vrfb_cfg non yuv dst_fi=%d.", vrfb->dst_fi);
		}
		DBG("vrfb_cfg w/h %d/%d\n", w, h);
	}

	src_paddr = vout->queue.bufs[buf_idx]->baddr;
	dst_paddr = vrfb->ctx[vrfb->next].paddr[rot] + vout->dss->foffset;

	DBG("dma chan %d buf %d src pa/%08x -> dst pa/%08x sz=%x\n", vrfb->dma_ch, buf_idx, src_paddr, dst_paddr, vrfb->size);
	...
	//omap_set_dma_src_burst_mode(vrfb->dma_ch, OMAP_DMA_DATA_BURST_8);
	...
	//omap_set_dma_dest_burst_mode(vrfb->dma_ch, OMAP_DMA_DATA_BURST_8);
...
static int cam_dss_update_overlay(struct omapvout_device *vout,
							int buf_idx)
{
	...
	o_info.color_mode = cam_dss_color_mode(vout->pix.pixelformat);
...
static void cam_dss_mark_buf_done(struct omapvout_device *vout, int idx)
{
...
static void cam_dss_perform_update(struct work_struct *work)
{
	...
	while (!list_empty(&vout->q_list)) {
		...
		if (dss->need_cfg) {
			rc = cam_dss_enable_transparency(vout);
			if (rc != 0) {
		...
		rc = cam_dss_perform_vrfb_dma(vout, idx, dss->need_cfg);
		if (rc != 0) {
			DBG("VRFB rotation failed %d\n", rc);
			goto failed_need_done;
		}

		cam_dss_mark_buf_done(vout, idx);

		rc = cam_dss_update_overlay(vout, idx);
		if (rc != 0) {
...
int  cam_dss_init(struct omapvout_device *vout, enum omap_plane plane)
{
	struct omap_overlay *ovly;
	int rc = 0;
	int i;
...
void cam_dss_remove(struct omapvout_device *vout)
{
	if (vout->dss != NULL) {
		cam_dss_release_vrfb(vout);
		kfree(vout->dss);
	}
}

int cam_dss_open(struct omapvout_device *vout, u16 *disp_w, u16 *disp_h)
{
	struct omap_dss_device *dev;
	int rc = 0;
	...
	INIT_WORK(&vout->dss->work, cam_dss_perform_update);
...
void cam_dss_release(struct omapvout_device *vout)
{
	/* It is assumed that the caller has locked the vout mutex */

	if (vout->dss->enabled)
		cam_dss_disable(vout);

	if (vout->dss && vout->dss->workqueue) {
		flush_workqueue(vout->dss->workqueue);
		destroy_workqueue(vout->dss->workqueue);
	}
}

bool cam_dss_is_rotation_supported(struct omapvout_device *vout)
{
	return vout->dss->vrfb.req_status == DMA_CHAN_ALLOTED;
}

int cam_dss_enable(struct omapvout_device *vout)
{
	/* It is assumed that the caller has locked the vout mutex */
...
void cam_dss_disable(struct omapvout_device *vout)
{
	int rc = 0;
	struct omap_overlay_info o_info;
	struct omap_overlay *ovly;
	...
	list_for_each_entry_safe(buf, tmp, &vout->q_list, queue) {
		list_del(&buf->queue);
		cam_dss_mark_buf_done(vout, buf->i);
	}

	rc = cam_dss_disable_transparency(vout);
	if (rc)
...
int cam_dss_update(struct omapvout_device *vout)
{
	/* It is assumed that the caller has locked the vout mutex */
