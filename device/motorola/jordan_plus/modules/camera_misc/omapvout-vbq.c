...
#define DEBUG 1
#undef DBG
#define DBG(fmt, args...) printk(KERN_INFO "vbq %s:"fmt, __func__, ##args)

/*=== Local Functions ==================================================*/

static void cam_vbq_vm_open(struct vm_area_struct *vma)
{
...
static void cam_vbq_vm_close(struct vm_area_struct *vma)
{
	struct omapvout_device *vout = vma->vm_private_data;
...
static struct vm_operations_struct vm_ops = {
	.open = cam_vbq_vm_open,
	.close = cam_vbq_vm_close
};

static void cam_acquire_frames(struct omapvout_device *vout,
							int cnt, int size)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		if (vout->bp &&	cam_bp_alloc(vout, size,
					&paddr, &vaddr, &fsize) == 0) {
		...
		{
#endif
			if (cam_mem_alloc(size, &paddr, &vaddr)) {
				DBG("Alloc failed %d\n", i);
				break;
			}
			fsize = size;
...
static void cam_release_frames(struct omapvout_device *vout)
{
	struct omapvout_vbq *vbq;
	int i;
	unsigned long paddr;
...
#ifdef CONFIG_VIDEO_OMAP_VIDEOOUT_BUFPOOL
		if (cam_is_bp_buffer(vout, paddr)) {
			if (cam_bp_release(vout, paddr))
				DBG("Error releasing to the pool\n");
		} else {
#else
		{
#endif
			cam_mem_free(paddr, vaddr, size);
		}
	}
...
static int cam_vbq_buf_setup(struct videobuf_queue *q,
			unsigned int *count, unsigned int *size)
{
	...
	cam_release_frames(vout);
	cam_acquire_frames(vout, cnt, sz);
...
static int cam_vbq_buf_prepare(struct videobuf_queue *q,
...
static void cam_vbq_buf_queue(struct videobuf_queue *q,
...
static void cam_vbq_buf_release(struct videobuf_queue *q,
			struct videobuf_buffer *vb)
{
	struct omapvout_device *vout;
	int i;
...
static struct videobuf_queue_ops vbq_buf_ops = {
	.buf_setup = cam_vbq_buf_setup,
	.buf_prepare = cam_vbq_buf_prepare,
	.buf_queue = cam_vbq_buf_queue,
	.buf_release = cam_vbq_buf_release
};

static void *cam_vbq_alloc(size_t size)
{
	return kzalloc(size, GFP_KERNEL);
}

static void *cam_vbq_vmalloc(struct videobuf_buffer *buf)
{
	/* Do nothing */
	return NULL;
}

static int cam_vbq_iolock(struct videobuf_queue *q,
...
static int cam_vbq_mmap_op(struct videobuf_queue *q, unsigned int *count,
...
static int cam_vbq_sync(struct videobuf_queue *q,
...
static int cam_vbq_copy_user(struct videobuf_queue *q, char __user *data,
...
static int cam_vbq_copy_strm(struct videobuf_queue *q, char __user *data,
...
static int cam_vbq_mmap_free(struct videobuf_queue *q)
{
	/* Do nothing */
	return 0;
}

static int cam_vbq_mmap_mapper(struct videobuf_queue *q,
					struct vm_area_struct *vma)
{
	...
	rc = cam_mem_map(vma, vout->vbq->buf[idx].phy_addr);
	if (rc != 0) {
...
static struct videobuf_qtype_ops vbq_ops = {
	...
	.alloc = cam_vbq_alloc,
	.vmalloc = cam_vbq_vmalloc,
	.iolock = cam_vbq_iolock,
	.mmap = cam_vbq_mmap_op,
	.sync = cam_vbq_sync,
	.video_copy_to_user = cam_vbq_copy_user,
	.copy_stream = cam_vbq_copy_strm,
	.mmap_free = cam_vbq_mmap_free,
	.mmap_mapper = cam_vbq_mmap_mapper
};

/*=== Public Interface Functions =========================================*/

int cam_vbq_init(struct omapvout_device *vout)
{
	struct omapvout_vbq *vbq;
	int rc;
...
void cam_vbq_destroy(struct omapvout_device *vout)
{
	/* It is assumed that the caller has locked the vout mutex */
...
int cam_vbq_buf_cnt(struct omapvout_device *vout)
{
	return vout->vbq->cnt;
}
