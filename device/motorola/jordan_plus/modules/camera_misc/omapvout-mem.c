...
int cam_mem_alloc(u32 size, unsigned long *phy_addr, void **virt_addr)
{
	void *page_addr;
	...
	*virt_addr = ioremap_cached(*phy_addr, size);
	...
	DBG("page/%08x; phy/%08lx; virt/%08x; size/%x\n",
...
void cam_mem_free(unsigned long phy_addr, void *virt_addr, u32 size)
{
	void *page_addr;
	...
	DBG("page/%08x; size/%x\n",
		(unsigned int) page_addr, size);

	iounmap(virt_addr);
}

int cam_mem_map(struct vm_area_struct *vma, unsigned long phy_addr)
{
	struct page *cpage;
	void *pos;
	u32 start;
