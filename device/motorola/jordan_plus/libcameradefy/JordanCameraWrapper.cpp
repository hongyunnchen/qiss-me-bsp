 ...
 */
/* { Updated by Jerry.Qian, 2012/03/27 /
 *
 *  https://github.com/Quarx2k/android_device_motorola_jordan
 *  git clone https://github.com/Quarx2k/android_device_motorola_jordan.git -b gingerbread 
 *
 * Updated by Jerry.Qian, 2012/03/27} */
...
/* { Added by Jerry.Qian, camera ANR if zoom in while taking picture, 2012/05/31 */
bool ZoomChanged = false ;
/* Added by Jerry.Qian, camera ANR if zoom in while taking picture, 2012/05/31 } */
/* { Added by Jerry.Qian, in auto flash mode, the flash led may light too, 2012/07/30 */
bool mMarkNotifyCB = false;//That the manual auto foucs may trigger a foucs-notify-callback. Try to mark it.
bool mChangedFlashMood = false;
/* Added by Jerry.Qian, in auto flash mode, the flash led may light too, 2012/07/30 } */
namespace android {
...
JordanCameraWrapper::dataCb(int32_t msgType, const sp<IMemory>& dataPtr, void* user)
{
    ...
    /* { Added by Jerry.Qian, camera ANR if zoom in while taking picture, 2012/05/31 */
    if( ZoomChanged && (msgType == CAMERA_MSG_COMPRESSED_IMAGE) ) {
        LOGD("notify that zoom is 0 after take picture");
        notifyCb(CAMERA_MSG_ZOOM, 0, 1, _this);
    }
    /* Added by Jerry.Qian, camera ANR if zoom in while taking picture, 2012/05/31 } */
...
/* { Added by Jerry.Qian, in auto flash mode, the flash led may light too, 2012/07/30 */
    if (msgType == CAMERA_MSG_RAW_IMAGE || msgType == CAMERA_MSG_COMPRESSED_IMAGE) {
        if( mChangedFlashMood ) {
            CameraParameters ret = _this->getParameters();
            ret.set(CameraParameters::KEY_FLASH_MODE, "auto");
            _this->setParameters(ret);
            mChangedFlashMood = false;
        }
    }
/* Added by Jerry.Qian, in auto flash mode, the flash led may light too, 2012/07/30 } */
 }
...
{
/* { Added by Jerry.Qian, in auto flash mode, the flash led may light too, 2012/07/30 */
    //That the manual auto foucs may trigger a foucs-notify-callback. Try to mark it.
    if( mMarkNotifyCB && !(msgType & CAMERA_MSG_FOCUS) ) {
...
/* { Added by Jerry.Qian, in auto flash mode, the flash led may light too, 2012/07/30 */
    //That the manual auto foucs may trigger a foucs-notify-callback. Try to mark it.
    if( mMarkNotifyCB && (msgType & CAMERA_MSG_FOCUS) ) {
        mMotoInterface->enableMsgType(CAMERA_MSG_FOCUS);
        mMarkNotifyCB = false;
    }
/* Added by Jerry.Qian, in auto flash mode, the flash led may light too, 2012/07/30 } */
...
JordanCameraWrapper::takePicture()
{
    /* { Added by Jerry.Qian, camera ANR if zoom in while taking picture, 2012/05/31 */
    if( ZoomChanged ) {
        if(mMotoInterface->msgTypeEnabled(CAMERA_MSG_ZOOM))
            LOGD("disableMsgType %d", CAMERA_MSG_ZOOM);
            mMotoInterface->disableMsgType(CAMERA_MSG_ZOOM);
    }
    /* Added by Jerry.Qian, camera ANR if zoom in while taking picture, 2012/05/31 } */
    /*
    *  { start------
    *  Added by Jerry.Qian, jpeg captured by green lens are black when infinity and flash on, 2012/06/08
    *  2012/07/30 ---- in auto flash mode, the flash led may light too.
    *
    */
    if( mInfinityAndFlashON && !mHaveDoneOnce ) {
        CameraParameters ret = mMotoInterface->getParameters();
        if( !strcmp(ret.get(CameraParameters::KEY_FLASH_MODE), "auto") ) {
            mChangedFlashMood = true;
            ret.set(CameraParameters::KEY_FLASH_MODE, "on");
            mMotoInterface->setParameters(ret);
        }

        //That the manual auto foucs may trigger a foucs-notify-callback. Try to mark it.
        if (mMotoInterface->msgTypeEnabled(CAMERA_MSG_FOCUS) ) {
            LOGE("Manual: disable Auto Foucs Message.");
            mMotoInterface->disableMsgType(CAMERA_MSG_FOCUS);
            mMarkNotifyCB = true;
        }

        mMotoInterface->autoFocus();
        mHaveDoneOnce = true;
    }
    /* end------ } */
...
JordanCameraWrapper::setParameters(const CameraParameters& params)
{
    ...
    /*
      ...
      */
    if(mCameraType == CAM_SOC) {
        if( strcmp(pars.get(CameraParameters::KEY_FOCUS_MODE),"infinity") ||
              ( strcmp(pars.get(CameraParameters::KEY_FLASH_MODE),"on") &
                 strcmp(pars.get(CameraParameters::KEY_FLASH_MODE),"auto"))
           )
            mInfinityAndFlashON= false;
        else
            mInfinityAndFlashON = true;
    }
    LOGD("Flash Mode: %s, Focus Mode: %s.", pars.get(CameraParameters::KEY_FLASH_MODE), pars.get(CameraParameters::KEY_FOCUS_MODE));
    /* end------ } */
    /* { Added by Jerry.Qian, , 2012/05/31 */
    //remove the custom parameter "cam-type"
    pars.remove("cam-type");

    //camera ANR if zoom in while taking picture
    if( (mCameraType == CAM_BAYER) && pars.getInt(CameraParameters::KEY_ZOOM) )
        ZoomChanged = true;
    else
        ZoomChanged = false;

    LOGD("CameraType %s, ZoomChanged %s.", mCameraType ? "CAM_BAYER" : "CAM_SOC" , ZoomChanged ? "true" : "false" );
    /* Added by Jerry.Qian, 2012/05/31 } */
...
JordanCameraWrapper::getParameters() const
{
	...
	/*
	  *{Added by Hobo.Shao, for SCR-00461,01/09/2012
	  *remove Green lens(SOC) 320x240 resolution
	 */
    	ret.set(CameraParameters::KEY_SUPPORTED_PICTURE_SIZES, "640x480,1280x960,1600x1200,2048x1536,2592x1456,2592x1936");
	/*	Added by Hobo.Shao, for SCR-00461,01/09/2012} */
    ...
    /* { Added by Jerry.Qian, add a custom parameter to make a distinction between red and green lens, 2012/05/31 */
    ret.set("cam-type", mCameraType ? "CAM_BAYER" : "CAM_SOC");
    /* Added by Jerry.Qian, add a custom parameter to make a distinction between red and green lens, 2012/05/31 } */
