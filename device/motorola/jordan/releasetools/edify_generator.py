...
# BEGIN Add by Seac.Cong for Defy( update boot.img and dev.tree) ,2012/03/21
  def UpdateBootImage(self, command):
    self.script.append('assert(package_extract_file("boot.img", "/tmp/boot.img"),')
    self.script.append('write_raw_image("/tmp/boot.img", "/dev/block/mmcblk1p15"),')
    self.script.append('delete("/tmp/boot.img"));')

  def UpdateDevTree(self, command):
    self.script.append('assert(package_extract_file("devtree.img", "/tmp/devtree.img"),')
    self.script.append('write_raw_image("/tmp/devtree.img", "/dev/block/mmcblk1p12"),')
    self.script.append('delete("/tmp/devtree.img"));')
#END Add by Seac.Cong for Defy( update boot.img and dev.tree) ,2012/03/21
