...
SYMSEARCH_DECLARE_FUNCTION_STATIC(void, class_create_release, struct class *);
...
static int find_pvr_class_struct(void)
{
	...
	/* { Added by Jerry.Qian, 20120727, start */
	/* The ram is dirty. It is a temp solution.*/
	/* defy kernel-gb08c3c8 on 0xdec913c0
	/* Added by Jerry.Qian, 20120227, end} */
	...
	for(i = 0; i < 0x30000000; i+=1)
	{
			...
			&& func[i+1] == match1 && func[i] == match0)
		{
			...
			if ((!strcmp("pvr", psPvrClass->name)) && (psPvrClass->class_release == class_create_release)) break;
...
static int unload_pvr_stack(void)
{
	...
	SYMSEARCH_BIND_FUNCTION(pvroff, class_create_release);
