                         ...
                         size_t bytes)
{
    struct legacy_stream_out *out =
        reinterpret_cast<struct legacy_stream_out *>(stream);
/* { Added by Jerry.Qian, audio play can't work, 2012/08/23 */
#ifdef HTC_ACE
//It's a temp solution.
//There is no crash when call another function(like usleep/LOGE) here. why?
    usleep(5);
#endif
/* Added by Jerry.Qian, audio play can't work, 2012/08/23 } */
...
{
/* { Modefied by Jerry.Qian, it's not supported by libaudio.so, 2012/08/30 */
...
/* { Modefied by Jerry.Qian, channel_mask should be changed if USES_AUDIO_LEGACY is used without ICS_AUDIO_BLOB, 2012/08/30 */
#ifndef ICS_AUDIO_BLOB
...
/* { Modefied by Jerry.Qian, channel_mask should be puted back when openOutputStream return without errors, 2012/08/30 */
#ifndef ICS_AUDIO_BLOB
#ifdef USES_AUDIO_LEGACY
    config->channel_mask = config->channel_mask >> 2;
#endif
