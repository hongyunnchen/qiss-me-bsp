                              ...
                              void* user)
{
    ...
    if((msg_type ==CAMERA_MSG_RAW_IMAGE)&&(dummy_raw_image_msg))
    	{
                                        ...
                                        const sp<IMemory>& dataPtr, void* user)
{
    ...
    gCameraHals[dev->cameraid]->releaseRecordingFrame(dataPtr);//Brian@  need release or record will stop
        ...
        struct preview_stream_ops *window)
{
    ...
    LOGV("%s---,rv %d", __FUNCTION__,0);
//Brian bug for some app start fail
    return 0;
}
...
void camera_enable_msg_type(struct camera_device * device, int32_t msg_type)
{
	...
	//Brian fix a lot of 3rd camera apk glitches end
...
int camera_start_recording(struct camera_device * device)
{
    int rv = -EINVAL;
    priv_camera_device_t* dev = NULL;
/*{Add by Hobo.Shao,for SCR-01979,03/16/2012
...
//Brian@  force start preview when recording stop
...
//Brian add for re-create the overlay
inline void destroyOverlay(struct camera_device * device) {
    priv_camera_device_t* dev = NULL;
    dev = (priv_camera_device_t*) device;
...
//Brian for barcode scan fail start
    ...
    if(preview_resolution_change && previewEnabled)
    	{
    	gCameraHals[dev->cameraid]->stopPreview();
		//Brian add for re-create the window&overlay,because window size changes
       destroyOverlay(device);
    	}
    ...
    if(preview_resolution_change && previewEnabled)
    	{
    	camera_set_preview_window(device, dev->window);
		//Brian add for re-create the window&overlay
...
char* camera_get_parameters(struct camera_device * device)
{
    ...
    // filter picture size
    //Brian@  set resolution crash fix
    camParams.set(CameraParameters::KEY_SUPPORTED_PICTURE_SIZES,
                  "3264x2448,3264x1952,2592x1552,2048x1536,1280x960,640x480");
	           //"3264x2448,2592x1936,2048x1536,1280x960,640x480");
	           
/*{Modify by Hobo.Shao,for SCR-01710,03/06/2012
*disable 960x720 preview size, avoid Panorama mode not fluent
*
*/
     camParams.set(CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES,
                  "1280x720,800x480,720x480,640x480,640x384,576x432,480x320");
/*Add by Hobo.Shao,for SCR-01710,03/06/2012}*/
...
/*
*{Added by Hobo.Shao, for SCR-03928,10/16/2012
*change thumbnail 800x480 to 640x480
*/
if ( !strcmp(camParams.get(CameraParameters::KEY_JPEG_THUMBNAIL_WIDTH), "800") && !strcmp(camParams.get(CameraParameters::KEY_JPEG_THUMBNAIL_HEIGHT), "480") ) {
    //LOGE("hhh: KEY_JPEG_THUMBNAIL_HEIGHT fixup");
      camParams.set(CameraParameters::KEY_JPEG_THUMBNAIL_WIDTH,
                   "640");
   }
/* Added by Hobo.Shao, for SCR-03928,10/16/2012}*/
		   
/*{Added by Hobo.Shao,for SCR-01821,04/24/2012
*force define preview fps is 30, improve Panorama mode not fluent
*
*/
    camParams.set(CameraParameters::KEY_SUPPORTED_PREVIEW_FRAME_RATES,
	  ...
	  "30");		
         ...
         "30,27,24,20,15");	
    #endif
/*Add by Hobo.Shao,for SCR-01821,04/24/2012}*/
/*
    camParams.set(CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES,
...
*/
    ...
    camParams.set(CameraParameters::KEY_PREVIEW_FRAME_RATE, "20");//Brian default
                ...
                hw_device_t** device)
{
    ...
    //android::Mutex::Autolock lock(gCameraDeviceLock);
//Brian@  not for other roms
    char value[PROPERTY_VALUE_MAX];
    if (!property_get("ro.build.id", value, NULL) || 
         strcmp(value, "QiSS_ME") || 
         (strlen(value)!=7) ||
         (value[4] != '_')
         ) {
        return -EINVAL;
