        ...
        case OMX_IndexParamVideoPortFormat:
            {
                ...
                if (pPortFormat->nPortIndex == pComponentPrivate->pInPortFormat->nPortIndex) {
                    ...
                    if(eError == OMX_ErrorNone) {
//Brian For MYUI-02197 & 02021 adobe flash halt start
			     //if((pComponentPrivate->pOutPortFormat->eColorFormat == VIDDEC_COLORFORMAT422) &&
				 //	(pComponentPrivate->pInPortFormat->nIndex == VIDDEC_DEFAULT_INPUT_INDEX_H264))//input avc  output 422 just for adobe flash
			     	{
			            switch (pPortFormat->nIndex) {
			            case VIDDEC_DEFAULT_OUTPUT_INDEX_INTERLEAVED422://return the output port format and type
			                break;
			            case VIDDEC_DEFAULT_OUTPUT_INDEX_PLANAR420://return the output port format and type
			                break;							
				     default://no more format support
					 	eError = OMX_ErrorNoMore;
			            }
			     	}
//Brian For MYUI-02197 & 02021 adobe flash halt end				
